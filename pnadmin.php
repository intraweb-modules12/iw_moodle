...
function iw_moodle_admin_conf() {
	...
	$countries_path = "modules/iw_moodle/pnlang/".pnusergetlang()."/countries.php";
	if(file_exists($countries_path)){
		include_once ($countries_path);
	} else {
		$countries_eng_path = "modules/iw_moodle/pnlang/eng/countries.php";
...
function iw_moodle_admin_sincron($args)
{
	...
	foreach($usuaris as $usuari){
		$userMDuid = pnModAPIFunc('iw_moodle', 'user', 'getuserMDuid', array('username' => $usersName[$usuari['uid']]));

		if($userMDuid['auth'] == 'db'){
			$userConnect = 1;
		}else{
			$userConnect = (pnModAPIFunc('iw_moodle', 'user', 'is_user', array('user' => $usersName[$usuari['uid']]))) ? 0 : -1;
		}
